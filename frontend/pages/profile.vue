<template>
  <div class="p-10">
    ...
    <form @submit.prevent="handleUpdateProfile" class="space-y-4 mt-4">
      ...
      <button type="submit" class="w-full p-2 bg-gray-500 text-white rounded hover:bg-gray-600">
        Update Profile
      </button>
    </form>
...
const handleUpdateProfile = async () => {
  try {
    const data = await updateUserProfile(username.value, password.value || undefined);
